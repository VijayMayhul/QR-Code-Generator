...
let getQRCode = ()=>{
    ...
    if(url == "" || url == undefined){
        ...
        fetch(`http://api.qrserver.com/v1/create-qr-code/?data=${url}!&size=400x400`)
        .then(response => {
            if (!response.ok) {
                throw new Error('Network response Error');
            }
        ...
        .then(blob => {
            ...
            let qrDiv = document.getElementById("qrDiv");
            buttonDownload.setAttribute("class", 'QrDiv bg-success p-0 d-flex justify-content-center align-items-center');
